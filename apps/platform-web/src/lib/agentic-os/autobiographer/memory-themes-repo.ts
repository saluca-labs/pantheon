...
interface RawMemoryThemeLinkRow {
  memory_id: string;
  theme_id: string;
...
interface RawMemoryAppearanceRow {
  memory_id: string;
  book_id: string | null;
  title: string;
...
function rowToLink(row: RawMemoryThemeLinkRow): MemoryThemeLink {
  return {
...
export async function listMemoriesForTheme(
  ...
  return r.rows.map((row: RawMemoryAppearanceRow) => ({
    memoryId: row.memory_id,
