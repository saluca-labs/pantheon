/**
 * Research coach persistence — single-table session storage.
 *
 * Mirrors Autobiographer / Maker OS Phase 7 exactly: one row per
 * session, whole transcript stored as an ordered JSONB array on the
 * row. No `messages` table — Research ships no mutating tools beyond
 * what the user can do through the existing notebook / hypothesis /
 * evidence routes, so the simpler one-table shape suffices.
 *
 * Cross-ownership is enforced at the SQL level: every read filters by
 * `user_id`. Experiment-scoped reads carry the `experiment_id` filter
 * but DO NOT join to `agos_research_experiments` (per the v0.1.30
 * platform contract, per-OS UUIDs are not FK-enforced on cross-cutting
 * tables). The session row owns its ownership check via `user_id`.
 *
...
interface RawResearchCoachSessionRow {
  id: string;
  user_id: string;
...
function rowToSession(row: RawResearchCoachSessionRow): CoachSession {
  return {
    id: row.id,
    userId: row.user_id,
...
const SESSION_COLUMNS = `id, user_id, experiment_id, mode, title, messages,
...
export async function createSession(
  ...
  const r = await pool.query(
    `INSERT INTO agos_research_coach_sessions
       (id, user_id, experiment_id, mode, title, messages, metadata)
...
export async function getSession(
  ...
  const r = await pool.query(
    `SELECT ${SESSION_COLUMNS}
       FROM agos_research_coach_sessions
...
export interface ListSessionsInput {
  ...
  /** Filter by mode. */
  mode?: CoachMode;
  /** Filter by exact experiment_id (use scope: 'workshop' for experiment_id IS NULL). */
  experimentId?: string;
  /** When 'workshop', filters to experiment_id IS NULL. */
  scope?: 'workshop' | 'experiment';
  limit?: number;
  offset?: number;
}
...
export async function updateSession(
  ...
  const pool = getResearchPool();
  await pool.query(
    `UPDATE agos_research_coach_sessions
...
export async function deleteSession(
  ...
  const pool = getResearchPool();
  const r = await pool.query(
    `DELETE FROM agos_research_coach_sessions
      WHERE id = $1 AND user_id = $2`,
    [id, userId],
  );
