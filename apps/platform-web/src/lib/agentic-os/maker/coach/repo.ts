/**
 * Maker coach persistence — single-table session storage.
 *
 * Unlike Filmmaker / Cyber (which split conversations from messages and
 * keep an action-log table), the Maker coach stores the whole transcript
 * on the session row as an ordered JSONB array. This is the simpler shape
 * locked by the Phase 7 spec — Maker ships no mutating tools, so there's
 * no action-log to keep.
 *
 * Cross-ownership is enforced at the SQL level: every read filters by
 * `user_id`. Project-scoped reads carry the `project_id` filter but DO
 * NOT join to `agos_maker_projects` (per the v0.1.30 platform contract,
 * per-OS UUIDs are not FK-enforced on cross-cutting tables). The session
 * row owns its ownership check via `user_id`.
 *
...
const SESSION_COLUMNS = `id, user_id, project_id, mode, title, messages,
...
/**
 * Insert a new session row. Mode is validated against the locked taxonomy;
 * an invalid mode is a programmer error (the route layer should validate
 * the body before reaching here).
 */
export async function createSession(
  ...
  const r = await pool.query(
    `INSERT INTO agos_maker_coach_sessions
       (id, user_id, project_id, mode, title, messages, metadata)
...
export async function getSession(
  ...
  const r = await pool.query(
    `SELECT ${SESSION_COLUMNS}
       FROM agos_maker_coach_sessions
...
export interface ListSessionsInput {
  ...
  /** Filter by mode. */
  mode?: CoachMode;
  /** Filter by exact project_id (use scope: 'workshop' for project_id IS NULL). */
  projectId?: string;
  /** When 'workshop', filters to project_id IS NULL. */
  scope?: 'workshop' | 'project';
  limit?: number;
  offset?: number;
}
...
export async function updateSession(
  ...
  const pool = getMakerPool();
  await pool.query(
    `UPDATE agos_maker_coach_sessions
...
export async function deleteSession(
  ...
  const pool = getMakerPool();
  const r = await pool.query(
    `DELETE FROM agos_maker_coach_sessions
      WHERE id = $1 AND user_id = $2`,
    [id, userId],
  );
