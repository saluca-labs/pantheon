...
/**
 * Filmmaker OS — Project Hub edit/delete actions.
 *
 * Renders the Edit button (opens an inline drawer to patch project metadata)
 * and the Delete button (confirm + cascade delete).
 *
...
export function ProjectHubActions({ project }: Props) {
  ...
  async function handleDelete() {
    setDeleting(true);
    try {
      const r = await fetch(`/api/tiresias/agentic-os/filmmaker/projects/${project.id}`, {
        method: 'DELETE',
      });
      if (!r.ok) throw new Error(`Delete failed (${r.status})`);
      router.push('/dashboard/os/filmmaker/projects');
      router.refresh();
    } catch {
      setDeleting(false);
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {editing && (
        <EditDrawer project={project} onClose={() => setEditing(false)} onSaved={() => { setEditing(false); router.refresh(); }} />
      )}
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Format">
            <select
              value={form.format}
              onChange={(e) => set('format', e.target.value as ProjectFormat)}
              className={inputCls}
            >
              {FORMATS.map((f) => (
