      ...
      {ids.length === 0 ? (
        ...
        <ul className="space-y-1">
          {ids.map((id) => (
            <li
              key={id}
              className="text-[10px] font-mono text-[#cbd5e1] truncate bg-[#0f1117] border border-[#2a2d3e] rounded px-2 py-1"
              title={id}
            >
              {id}
            </li>
...
export function CoachContextPanel({ mode, experimentId, messages }: Props) {
  ...
  // methods_advisor
  return (
    <aside className="rounded-xl border border-[#2a2d3e] bg-[#1a1d27] p-4 self-start max-h-[calc(100vh-200px)] overflow-y-auto space-y-3">
      <div className="flex items-center gap-1.5">
        <ShieldCheck className="w-3.5 h-3.5 text-[#4361EE]" />
        <h3 className="text-xs font-semibold text-white uppercase tracking-wide">
          Experiment scope
        </h3>
      </div>
      <p className="text-[10px] font-mono text-[#cbd5e1] truncate bg-[#0f1117] border border-[#2a2d3e] rounded px-2 py-1">
        {experimentId ?? '(none)'}
      </p>
