/**
 * Maker coach — streaming chat session view.
 *
 ...
 * for the session id (no redaction flag — Maker is low-harm). Provides
 * a delete + rename affordance on the conversation header.
 *
...
export function CoachSession({
  ...
  async function send() {
    ...
    try {
      const r = await fetch(
        `/api/tiresias/agentic-os/maker/coach/sessions/${sessionId}/messages`,
        {
          method: 'POST',
      ...
      // Wave-0 migration: route handler now returns JSON instead of an
      // SSE-style stream. Streaming UX is deferred until @platform/llm
      // exposes streamLlm to the route layer.
  ...
  async function deleteSession() {
    ...
    const r = await fetch(
      `/api/tiresias/agentic-os/maker/coach/sessions/${sessionId}`,
      { method: 'DELETE' },
    );
    if (r.ok) {
      router.push('/dashboard/os/maker/coach');
    }
  }
    ...
    <div className="flex flex-col h-[calc(100vh-200px)]">
        ...
        <div className="flex items-center gap-3 min-w-0">
          {editingTitle ? (
            <input
              ...
              autoFocus
              className="bg-transparent text-lg font-semibold text-white border-b border-os-maker focus:outline-none px-1"
            />
          ) : (
            <h2
          ...
          <span className="text-[10px] font-medium uppercase tracking-wide px-2 py-0.5 rounded-full border border-os-maker/30 bg-os-maker/10 text-os-maker shrink-0">
      ...
      >
        <textarea
          ...
          className="flex-1 rounded-lg border border-border-subtle bg-surface-2 text-sm text-white placeholder:text-text-tertiary px-3 py-2 focus:outline-none focus:border-os-maker disabled:opacity-50"
        />
        <button
          type="submit"
          disabled={streaming || !text.trim()}
          className="inline-flex items-center gap-1.5 rounded-lg bg-os-maker hover:bg-os-maker/90 text-black text-sm font-medium px-3 py-2 disabled:opacity-50"
        >
