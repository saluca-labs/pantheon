...
import { Send, ShieldCheck, Trash2, Wrench } from 'lucide-react';
import {
  COACH_MODE_LABELS,
  type CoachMode,
...
interface ChatTrailer {
  conversation_id?: string;
  redacted?: boolean;
  redaction_matches?: RedactionMatch[];
}
...
export function CoachChat({
  ...
  async function send() {
    ...
    const assistantPlaceholderId = `local-${crypto.randomUUID()}`;
    ...
    try {
      const r = await fetch(
        `/api/tiresias/agentic-os/cyber/coach/conversations/${conversationId}/chat`,
        {
          method: 'POST',
  ...
  async function deleteConversation() {
    ...
    const r = await fetch(
      `/api/tiresias/agentic-os/cyber/coach/conversations/${conversationId}`,
      { method: 'DELETE' },
    );
    if (r.ok) {
      router.push(`/dashboard/os/cyber/coach`);
    }
  }
    ...
    <div className="flex flex-col h-[calc(100vh-200px)]">
      ...
      >
        <textarea
          ...
          className="flex-1 rounded-lg border border-[#2a2d3e] bg-[#1a1d27] text-sm text-white placeholder:text-[#64748b] px-3 py-2 focus:outline-none focus:border-red-400 disabled:opacity-50"
        />
        <button
          type="submit"
...
function CoachMessageBubble({ message }: { message: CoachUiMessage }) {
  ...
  const redactedTypes = (message.redactionMatches ?? [])
    .map((m) => m.type)
    .join(', ');
  return (
