    ...
    <div className="rounded-xl border border-[#2a2d3e] bg-[#1a1d27] p-4">
      <div className="flex items-start justify-between gap-3 flex-wrap mb-2">
        <div className="flex items-center gap-2 min-w-0">
          <Hash className="w-4 h-4 text-[#4361EE] shrink-0" />
          <span className="text-sm font-mono text-white truncate" title={ioc.value}>
            {ioc.value}
          </span>
        </div>
        <div className="flex items-center gap-1.5 shrink-0">
          <span className="text-[10px] uppercase tracking-wide px-2 py-0.5 rounded-full border border-[#2a2d3e] text-[#94a3b8]">
            {ioc.kind}
          </span>
          {ioc.threatType && (
      ...
      {ioc.description && (
        <p className="text-xs text-[#cbd5e1] mb-2">{ioc.description}</p>
      )}
