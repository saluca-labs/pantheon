    ...
    <div className="space-y-6">
      ...
      <section className="rounded-xl border border-[#2a2d3e] bg-[#1a1d27] p-5 space-y-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3 text-xs">
          <div>
            <span className="text-[#94a3b8] uppercase tracking-wide mr-1.5">Type</span>
            <span className="text-white">{organization.orgType.replace(/_/g, ' ')}</span>
          </div>
          {organization.industry && (
            <div>
              <span className="text-[#94a3b8] uppercase tracking-wide mr-1.5">Industry</span>
              <span className="text-white">{organization.industry}</span>
            </div>
          )}
          {organization.website && (
            <div className="sm:col-span-2">
              <span className="text-[#94a3b8] uppercase tracking-wide mr-1.5">Website</span>
              <a
      ...
      <section className="rounded-xl border border-[#2a2d3e] bg-[#1a1d27] p-5">
        <h2 className="text-sm font-semibold text-white mb-3">
          People <span className="text-[#94a3b8] font-normal">({people.length})</span>
        </h2>
        {people.length === 0 ? (
          <p className="text-xs text-[#94a3b8]">No people linked to this organization yet.</p>
        ) : (
            ...
            {people.map((p) => (
              <li key={p.id}>
                <Link
                  ...
                  className="block rounded-md border border-[#2a2d3e] p-2.5 hover:border-[#4361EE] transition"
                >
                  <p className="text-sm text-white">{fullName(p)}</p>
                  {p.role && <p className="text-[11px] text-[#94a3b8]">{p.role}</p>}
                </Link>
              </li>
            ))}
          </ul>
