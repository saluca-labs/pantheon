    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mt-4">
      <div className="lg:col-span-2 space-y-5">
        ...
        <div className="rounded-xl border border-border-subtle bg-surface-2 p-5">
          ...
          {(recipe.ingredients ?? []).length === 0 ? (
            <p className="text-xs text-text-secondary">No ingredients yet.</p>
          ) : (
            <ul className="space-y-1.5 text-sm text-text-primary">
              {(recipe.ingredients ?? []).map((i) => (
                <li
                  key={i.id}
                  className="flex items-start justify-between gap-3 border-b border-border-subtle py-1.5 last:border-b-0"
                >
                  <span>
      ...
      <aside>
        <div className="rounded-xl border border-border-subtle bg-surface-2 p-5 sticky top-4">
          ...
          {!nutrition ? (
            <p className="text-xs text-text-secondary">No nutrition rollup yet.</p>
          ) : (
