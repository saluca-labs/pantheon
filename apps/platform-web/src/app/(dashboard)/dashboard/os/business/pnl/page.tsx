 ...
 *
 * Wave D (UI Depth Wave) specialization: the live P&L summary now renders as
 * a `DashboardWidget` + `ChartCard` grid (see `PnlSummaryPanel`). This page
 * keeps the snapshot filter chips + list; legacy hex literals on the chips +
 * back-link are migrated onto the visual-language tokens.
 *
    ...
    <div className="max-w-5xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <BarChart3 className="w-6 h-6 text-os-business" />
          <h1 className="text-2xl font-semibold text-text-primary">P&L</h1>
        </div>
      </div>

      {/* Live P&L summary — DashboardWidget + ChartCard grid */}
      <div className="mb-8 rounded-xl border border-border-subtle bg-surface-2 p-6">
        <PnlSummaryPanel userId={user.userId} />
      </div>

      {/* Snapshots section */}
      <div className="mb-6">
        <h2 className="text-lg font-medium text-text-primary mb-4">Snapshots</h2>
        ...
        <div className="flex items-center gap-2 mb-4 overflow-x-auto pb-1">
          {PERIOD_FILTERS.map((k) => (
            <Link
              ...
              className={`inline-flex items-center rounded-md border px-3 py-1.5 text-xs font-medium transition whitespace-nowrap ${
                activePeriodKind === k
                  ? 'border-accent bg-accent/10 text-accent'
                  : 'border-border-subtle bg-surface-2 text-text-secondary hover:text-text-primary hover:border-accent/50'
              }`}
            >
              {k === 'all' ? 'All' : k}
            </Link>
          ...
          <Link
            ...
            className={`inline-flex items-center rounded-md border px-3 py-1.5 text-xs font-medium transition whitespace-nowrap ${
              lockedOnly
                ...
                : 'border-border-subtle bg-surface-2 text-text-secondary hover:text-text-primary hover:border-warning/50'
            }`}
          >
            Locked only
          </Link>
