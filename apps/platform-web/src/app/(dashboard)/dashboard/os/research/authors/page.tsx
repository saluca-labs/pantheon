          ...
          {sortedBuckets.map(([bucket, list]) => (
            <section key={bucket} aria-labelledby={`authors-bucket-${bucket}`}>
              <h2
                ...
                className="text-sm font-semibold text-[#94a3b8] uppercase tracking-wide mb-2"
              >
                {bucket}
              </h2>
                ...
                {list.map((a) => (
                  <li key={a.id}>
                    <Link
                      ...
                      className="flex items-center justify-between gap-3 px-3 py-2 rounded-lg border border-[#2a2d3e] bg-[#1a1d27] hover:border-[#4361EE]/40 transition"
