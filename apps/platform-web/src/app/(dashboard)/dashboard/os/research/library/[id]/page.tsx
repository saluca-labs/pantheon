  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="rounded-xl border border-[#2a2d3e] bg-[#1a1d27] p-6 mb-6">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap mb-2">
              ...
              {paper.venue && (
                <span className="text-xs text-[#94a3b8]">{paper.venue}</span>
              )}
              {paper.archivedAt && (
            ...
            <div className="mt-3 flex items-center gap-3 flex-wrap text-xs text-[#94a3b8]">
              {paper.doi && (
                <span>
        ...
        <div className="mt-4">
          <p className="text-[10px] text-[#94a3b8] uppercase tracking-wide mb-2">Authors</p>
          <AuthorChipList
            authors={authors}
        ...
        {paper.tags.length > 0 && (
          <div className="mt-4">
            <p className="text-[10px] text-[#94a3b8] uppercase tracking-wide mb-2">Tags</p>
      ...
      <section className="mb-6">
        <h2 className="text-sm font-semibold text-white uppercase tracking-wide mb-2 inline-flex items-center gap-2">
          <ScrollText className="w-4 h-4 text-[#4361EE]" />
          Abstract
        </h2>
            ...
            {linkingExperiments.map((row) => (
              ...
              >
                <div className="min-w-0">
                  ...
                  {row.link.notes && (
                    <p className="text-[10px] text-[#94a3b8] mt-1">{row.link.notes}</p>
                  )}
                </div>
                <span className="text-[10px] uppercase tracking-wide px-1.5 py-0.5 rounded-full border border-[#4361EE]/40 bg-[#4361EE]/15 text-[#cbd5e1] shrink-0">
              ...
              >
                <div className="flex items-start justify-between gap-2 flex-wrap">
                  <div className="min-w-0">
                    ...
                    {row.notes && (
                      <p className="text-[10px] text-[#94a3b8] mt-1">{row.notes}</p>
                    )}
                  </div>
                  <span className="text-[10px] uppercase tracking-wide px-1.5 py-0.5 rounded-full border border-[#2a2d3e] bg-[#0f1117] text-[#cbd5e1] shrink-0">
                    {row.polarity}
                  </span>
                </div>
