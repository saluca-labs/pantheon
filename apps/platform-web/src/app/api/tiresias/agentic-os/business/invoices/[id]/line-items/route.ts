/**
 * Business OS Phase 4 — invoice line-items collection route.
 *
 * GET  /api/tiresias/agentic-os/business/invoices/[id]/line-items
 * POST /api/tiresias/agentic-os/business/invoices/[id]/line-items
 *
...
export async function GET(_req: NextRequest, { params }: Props) {
  ...
  const items = await listLineItems('invoice', id, user.userId);
...
export async function POST(request: NextRequest, { params }: Props) {
  ...
  await recordAudit({
    ...
    payload: { lineItemId: item.id, parentType: 'invoice', parentId: id },
  });
