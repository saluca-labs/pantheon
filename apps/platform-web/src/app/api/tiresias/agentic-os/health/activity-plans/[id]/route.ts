...
import { ActivityPlanUpdateBody } from '@/lib/agentic-os/health/schemas';
...
export async function GET(_request: NextRequest, { params }: RouteCtx) {
  ...
  const plan = await getActivityPlan(id, ok.user.tenantId, ok.user.userId);
  if (!plan) {
...
export async function PATCH(request: NextRequest, { params }: RouteCtx) {
  ...
  const ok = await ensureUserAndConsent();
  if ('err' in ok) return ok.err;
  const parsed = ActivityPlanUpdateBody.safeParse(
  ...
  const updated = await withCrisisGuard(
    ...
    {
      osSlug: 'health',
      source: 'health.activity_plan.update',
    ...
    () => updateActivityPlan(id, ok.user.tenantId, ok.user.userId, parsed.data),
  );
  if (!updated) {
    return NextResponse.json({ error: 'Not found' }, { status: 404 });
  }
  await recordAudit({
    ...
    action: 'health.activity_plan.update',
...
export async function DELETE(_request: NextRequest, { params }: RouteCtx) {
  ...
  const ok = await ensureUserAndConsent();
  if ('err' in ok) return ok.err;
  const deleted = await deleteActivityPlan(
    id,
    ok.user.tenantId,
    ok.user.userId,
  );
  ...
  await recordAudit({
    actorId: ok.user.userId,
    action: 'health.activity_plan.delete',
    payload: { id },
  });
