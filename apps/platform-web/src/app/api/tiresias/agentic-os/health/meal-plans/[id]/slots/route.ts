...
import { MealPlanSlotBody } from '@/lib/agentic-os/health/schemas';
...
export async function POST(request: NextRequest, { params }: RouteCtx) {
  ...
  const parsed = MealPlanSlotBody.safeParse(
  ...
  const created = await withCrisisGuard(
    ...
    {
      osSlug: 'health',
      source: 'health.meal_plan.slot.create',
    ...
    () =>
      addPlanSlot(planId, ok.user.tenantId, ok.user.userId, parsed.data),
  );
  if (!created) {
  ...
  await recordAudit({
    ...
    action: 'health.meal_plan.slot.create',
    payload: { planId, slotId: created.id, dayOfWeek: created.dayOfWeek, mealSlot: created.mealSlot },
  });
