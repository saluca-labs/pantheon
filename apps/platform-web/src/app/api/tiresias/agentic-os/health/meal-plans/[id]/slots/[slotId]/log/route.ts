...
/**
 * POST [?date=YYYY-MM-DD] — log this planned slot as an actual meal entry
 * on the given date (defaults to today UTC).
 */
export async function POST(request: NextRequest, { params }: RouteCtx) {
  ...
  const entry = await addPlanSlotToMealLog(
    slotId,
    ok.user.tenantId,
    ok.user.userId,
    date,
  ...
  await recordAudit({
    ...
    action: 'health.meal_plan.slot.log',
    payload: { planId, slotId, mealEntryId: entry.id, date },
  });
