...
import { ActivityPlanBody } from '@/lib/agentic-os/health/schemas';
...
export async function GET(request: NextRequest) {
  ...
  if (week) {
    const plan = await getActivityPlanForWeek(
      ok.user.tenantId,
      ok.user.userId,
      week,
    );
  ...
  const plans = await listActivityPlans({
...
export async function POST(request: NextRequest) {
  const ok = await ensureUserAndConsent();
  if ('err' in ok) return ok.err;
  const parsed = ActivityPlanBody.safeParse(
  ...
  try {
    const created = await withCrisisGuard(
      ...
      {
        osSlug: 'health',
        source: 'health.activity_plan.create',
      ...
      () => createActivityPlan(ok.user.tenantId, ok.user.userId, parsed.data),
    );
    await recordAudit({
      actorId: ok.user.userId,
      action: 'health.activity_plan.create',
      payload: { id: created.id, weekStartDate: created.weekStartDate },
    });
    return NextResponse.json({ plan: created }, { status: 201 });
  } catch (e) {
    if (e instanceof ActivityPlanValidationError) {
