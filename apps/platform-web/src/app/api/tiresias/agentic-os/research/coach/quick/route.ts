/**
 * Research coach — one-shot quick prompt (no persistence).
 *
 ...
 * missing.
 *
 * methods_advisor still requires an experiment_id here — the contract
 * holds across the create and quick surfaces. The regulated-advice
 * detection layer runs over the supplied message and primes the
 * referral footer just like the messages route.
 *
 * Note: this route does NOT audit (no durable artifact, nothing to
 * audit-trail) and does NOT verify experiment ownership directly — the
 * context loader will throw if the user passes an experimentId they
 * don't own and that bubbles up as a 400 here.
 *
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    systemPrompt = buildSystemPrompt(built.context, mode, parsed.data.message);
