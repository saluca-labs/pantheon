...
| `INTERNAL_API_KEY` | `***REMOVED***` (plaintext!) | missing | ADD as secretKeyRef (Step 2.5) |
...
The key `***REMOVED***` is currently a plaintext env var in production. It must be moved to the k8s Secret before the new manifest is applied.
...
kubectl patch secret tiresias-secrets -n tiresias \
  --type='json' \
  -p='[{"op":"add","path":"/data/internal-api-key","value":"***REMOVED***"}]'
...
# Expected: INTERNAL_API_KEY=***REMOVED***
