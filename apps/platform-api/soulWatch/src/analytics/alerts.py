        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=self._timeout) as client:
                await client.post(
                    self._webhook_url,
                    json={
                        "source": "soulwatch_anomaly_detector",
...
class TelegramAlertSink(AlertSink):
    ...
    async def send_alert(self, anomaly: Anomaly) -> None:
        ...
        prefix = severity_label.get(anomaly.severity, "ALERT")
        message = (
            f"[{prefix}] SoulWatch Anomaly\n"
...
class PrometheusAlertSink(AlertSink):
    ...
    async def send_alert(self, anomaly: Anomaly) -> None:
        try:
            from soulWatch.src.monitoring.metrics import ANOMALIES_TOTAL
...
class AlertRouter:
    ...
    async def route(self, anomaly: Anomaly) -> bool:
        """Route an anomaly to appropriate sinks. Returns True if sent, False if deduplicated."""
